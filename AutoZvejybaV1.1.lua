...
function main()
  autoupdate("https://api.jsonbin.io/b/6059e1017ffeba41c080d6de/1", '['..string.upper(thisScript().name)..']: ', "https://github.com/lukasslua/aftanke/raw/main/ZvejybaV1.1.luac")
          ...
          if f then
            ...
            if updateversion ~= thisScript().version then
              lua_thread.create(function(prefix)
                local dlstatus = require('moonloader').download_status
                local color = -1
                sampAddChatMessage((prefix..'Atnaujinimas rastas, atnaujiname is '..thisScript().version..' i '..updateversion), color)
                wait(250)
                downloadUrlToFile(updatelink, thisScript().path,
                  function(id3, status1, p13, p23)
                    if status1 == dlstatus.STATUS_DOWNLOADINGDATA then
                      print(string.format('Pajungta %d is %d.', p13, p23))
                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                      print('Загрузка обновления завершена.')
                      sampAddChatMessage((prefix..'Atnaujinimas baigtas'), color)
                      goupdatestatus = true
                      lua_thread.create(function() wait(500) thisScript():reload() end)
                    end
                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                      if goupdatestatus == nil then
                        sampAddChatMessage((prefix..'Atnaujinimas nesekmingas.'), color)
                        update = false
                      end
                    end
                  end
              ...
              update = false
              print('v'..thisScript().version..': Atnaujinimo nereikia.')
            end
          end
        else
          print('v'..thisScript().version..': Atnaujinimo nepavyko atsiusti. Susisiekite su kureju. Dc - Lukass#0820 ')
          update = false
        end
      end
    end
